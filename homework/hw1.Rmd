...
The size of git_experiment/ folder is 3.8M and the total size of the individual files inside the folder is 66K. It happened when the hidden folder .git kept 
tracking all the log information, commits and the corresponding snapshot of the changed files in each loop. Therefore, it's always recommended to not commit large binary files since the memory required to keep track of those large files would be much larger.
...
Hyperlink to the `hiperglm` repository on your Github account: https://github.com/graced03/hiperglm.git
