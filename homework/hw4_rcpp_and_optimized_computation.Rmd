...
**Answer**
When I ran the code for multiple times, there was always an error, the value of the second inner prod changed but was never the correct answer.
It happens when the buggy code retrieves values in the vector from index 1 to index n (v.size(), w.size()). However, in R, it stores the vector from indices 0 to n-1. Therefore, the last value retrieved points to an un-allocated space outside of the indicated memory space for vector v or w, leading to the error of the inner product.
...
**Answer**
The output of the above code is a 4x2 matrix with all numbers in the first column to be 1024, and all numbers in the the second column to be a very large number 2.769776e+282. When I re-ran the code, the first column remained the same, but the second column will change to a different number and sometimes it gives out NaN. It happens given that R stores vector in the column-major order, so that matrix B is stored as a 1024 dimension vector where each element is 1. Therefore, we the computer does the matrix multiplication, it gets 1024 in the first column. And for the second column, the computer points to unallocated space so that the value changes every time when we re-ran it and it sometimes gives NaN.
...
**Answer**
The R session aborted when I tried to run the first code chunk. It happens when the two vectors are of different dimension and the code tried to modify y on the memory that was not allocated to y (y is of dimension 1000 and when i>=1000, the program was not able to find y[i]).

The second code chunk ran fine given that the dimension of y is larger than x. So that when i goes from 0 to 999, it is within the memory allocated to y, but it only gives expected results on the first 1000 elements.

Here we added one line to check the dimension of the two vectors before the calculation.
...
base_out <- A %*% B
row_col_matmat_out <- row_col_dot_matmat(A,B)
are_all_close(row_col_matmat_out, base_out)

```

Here we tested with the are_all_close function and proved that the two outputs agreed.
...
**Answer**
The `col_oriented_matmat`, which took 189ms (min and median) is faster than `row_col_dot_matmat`, which took 2.94s (min and median). It happens given that matrices are stored in the column major order. As we discussed in the class, to access the matrix by row, the program needs to jump from one spot to another, which takes longer time than when we access the whole column by moving to the element stored next.

```{r}
# Check and benchmark
col_oriented_matmat_out <- col_oriented_matmat(A,B)
are_all_close(col_oriented_matmat_out, base_out)
...
**Answer**
Here we found that using the `col_oriented_matmat` is 2 times faster than the base R matrix multiplication function.
...
**Answer**
The following answer given are generated with MacBook Air Early 2015 with Intel Core i5 processor. 

| Complier   | Eigen | Col   | default| 
| -------  - |:-----:| -----:|-------:|
| SSE        | 352ms | 1s    | 1.45s  |
| AVX        | 945ms | 1.31s | 1.29s  |
| FMA        | 904ms |  1.2s | 1.46s  |
| AVX SSE    | 128ms | 793ms | 1.29s  |
| FMA SSE    | 77.2ms| 822ms | 1.24s  |
| AVX FMA    | 831ms | 1.22s | 1.24s  |
| AVX FMA SSE| 78.8ms| 696ms | 1.32s  |

From the above table, the `dgemm_eigen()` function is the fastest, while the column-oriented calculation is usually faster than the default method `%*%` in R except we we only turn on AVX. Paired FMA with SSE, we achieved the fastest computation using the `dgemm_eigen()` function. We can conclude that using FMA where the floating-point operation performs multiplication and addition in one step speeds up the matrix-matrix multiplication a lot.
